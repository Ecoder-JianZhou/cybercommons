...
def prepareData(firstYr, endYr, outDirName):
    outPath = "output/process-based-TECO_output"
    # GPP --------------------------------------------------
    os.makedirs(os.path.join(outDirName,"gpp"), exist_ok = True)
    df_gpp = pd.read_csv(outPath+"/Simu_dailyflux14001.txt")
    df_gpp.columns = ["sdoy", "GPP_d", "NEE_d", "Reco_d", "NPP_d", "Ra_d", "QC1", "QC2", "QC3", "QC4", "QC5", "QC6", "QC7", "QC8", "Rh_d"]
    ...
    df_stemp = pd.read_csv(outPath+"/Simu_soiltemp.txt",header=None)
    ...
    dat_stemp.to_csv(os.path.join(outDirName,"stemp")+"/stemp_day.csv",index_label="Date")
    dat_stemp_m = dat_stemp.resample('M').mean()
    dat_stemp_m.to_csv(os.path.join(outDirName,"stemp")+"/stemp_month.csv",index_label="Date")

    # smoist
    os.makedirs(os.path.join(outDirName,"smoist"), exist_ok = True) # create output path of smoist
    df_smoist = pd.read_csv(outPath+"/Simu_dailywater001.txt")#,header=None)
    ...
    dat_smoist.to_csv(os.path.join(outDirName,"smoist")+"/smoist_day.csv",index_label="Date")
    dat_smoist_m = dat_smoist.resample('M').mean()
    dat_smoist_m.to_csv(os.path.join(outDirName,"smoist")+"/smoist_month.csv",index_label="Date")
    ...
    df_data = pd.read_csv(cliPath+"/SPRUCE_forcing_plot17.txt","\t")
...
if __name__=="__main__":
    prepareData(2015,2021,"output/matrix_models_output")